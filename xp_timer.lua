...
local function create_ui()
    ...
    frame:SetScript("OnDragStop", function(self)
        ...
        xpt_character_data.ui_point = point
        xpt_character_data.ui_relPoint = relPoint
        xpt_character_data.ui_xOfs = xOfs
        xpt_character_data.ui_yOfs = yOfs
        -- update global last position for new characters
        xpt_global_data.last_ui_point = point
        xpt_global_data.last_ui_relPoint = relPoint
        xpt_global_data.last_ui_xOfs = xOfs
        xpt_global_data.last_ui_yOfs = yOfs
    end)
    ...
    if not xpt_character_data.ui_point then
        ...
        if xpt_global_data.last_ui_point then
            xpt_character_data.ui_point = xpt_global_data.last_ui_point
            xpt_character_data.ui_relPoint = xpt_global_data.last_ui_relPoint
            xpt_character_data.ui_xOfs = xpt_global_data.last_ui_xOfs
            xpt_character_data.ui_yOfs = xpt_global_data.last_ui_yOfs
        else
            -- default to center if no position data is available
            xpt_character_data.ui_point = "CENTER"
            xpt_character_data.ui_relPoint = "CENTER"
            xpt_character_data.ui_xOfs = 0
            xpt_character_data.ui_yOfs = 0
        end
    end
...
local function create_options_panel()
    ...
    panel.cashTimeEdit:SetScript("OnEditFocusLost", function(self)
        ...
        if val and val > 0 and val <= 1440 then
            ...
            self:SetText(xpt_global_data.ui_timeframe or xpt_global_data_defaults.ui_timeframe)
        end
    end)
    ...
    panel.resetPosition:SetScript("OnClick", function()
        ...
        if xpt_ui_frame then
            ...
            xpt_ui_frame:SetPoint(xpt_character_data.ui_point, UIParent, xpt_character_data.ui_relPoint,
                xpt_character_data.ui_xOfs, xpt_character_data.ui_yOfs)
        end
...
function xpt:ADDON_LOADED(...)
    ...
    if addon == "xp_timer" then
        ...
        if xpt_global_data.ui_timeframe == nil then xpt_global_data.ui_timeframe = xpt_global_data_defaults.ui_timeframe or
            5 end
    ...
    if (time_diff > 0) then
        if (xpt_global_data.show_time_on_xp) then
            ...
            if estimate_inacuracy > 60 then
                xpt:print(
                string.format("Time to next level down to: |cff00ff00%s|r",
                    xp_util.to_hms_string(self.time_till_next_level)), 0.38, 0.58, 0.92);
            else
                if estimate_inacuracy < -60 then
                    xpt:print(
                    string.format("Time to next level increasing: |cffff0000%s|r kill faster or change zones",
                        xp_util.to_hms_string(self.time_till_next_level)), 0.38, 0.58, 0.92);
                else
                    xpt:print(string.format("Time to next level: %s", xp_util.to_hms_string(self.time_till_next_level)),
                        0.38, 0.58, 0.92);
                end
            end
        end
...
function xpt:default()
    ...
    xpt:print("XP gains same as last XP gain to next level: " .. kills_to_lvl);
    xpt:print(string.format("Time to next level: |cffff0000%s|r",
        xp_util.to_hms_string((UnitXPMax("player") - xp_cur) / xp_per_second)));
end
